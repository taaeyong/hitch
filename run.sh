...
mkdir -p hitch tests data/demo-run/relationships data/demo-run/ingest data/demo-run/wiki data/demo-run/simulation data/demo-run/graph data/demo-run/telegram raw

if [[ -f girlfriend_kakaotalk.csv && ! -f raw/girlfriend_kakaotalk.csv ]]; then
  cp girlfriend_kakaotalk.csv raw/girlfriend_kakaotalk.csv
fi
if [[ -f mother_kakaotalk.csv && ! -f raw/mother_kakaotalk.csv ]]; then
  cp mother_kakaotalk.csv raw/mother_kakaotalk.csv
fi

cat > hitch/__init__.py <<'EOF'
"""Hitch local rehearsal runtime package."""
EOF

cat > hitch/demo.py <<'EOF'
...
bash ./run.sh >/tmp/hitch-v4-run.log 2>&1 || true
bash ./loop.sh rehearse